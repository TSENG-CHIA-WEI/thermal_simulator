...
def main():
    ...
    if hasattr(solver, 'residual_history') and len(solver.residual_history) > 0:
        ...
        print(f"\n[Convergence] Iterations: {len(hist)}, Final Residual: {hist[-1]:.6e}")
        
        # Save CSV
        conv_csv = os.path.join(os.getcwd(), 'convergence_history.csv')
        np.savetxt(conv_csv, np.column_stack([range(1, len(hist)+1), hist]),
                   delimiter=',', header='Iteration,Residual', comments='')
        
        # Plot curve
        fig, ax = plt.subplots(figsize=(10, 6), dpi=150)
        ax.semilogy(range(1, len(hist)+1), hist, 'bo-', linewidth=2, markersize=4)
        ax.axhline(y=1e-6, color='r', linestyle='--', linewidth=1.5, label='Target (1e-6)')
        ax.set_xlabel('Iteration', fontsize=12, fontweight='bold')
        ax.set_ylabel('Residual ||r||', fontsize=12, fontweight='bold')
        ax.set_title('CG Solver Convergence', fontsize=14, fontweight='bold')
        ax.grid(True, which='both', linestyle=':', alpha=0.6)
        ax.legend()
        plt.tight_layout()
        conv_png = os.path.join(os.getcwd(), 'convergence_curve.png')
        plt.savefig(conv_png, dpi=150)
        plt.close()
